...
parser.add_argument("-np", help="extract 1-D spectra from nth pixel", dest="np", default = False)
parser.add_argument("-ow", help="True = overwrite, False = Don't overwrite", dest="overwrite", default = False)
parser.add_argument("-fit_size", help="aperture size in pixels for Gaussian fit, choose any number between 50 and 200", dest="fit_size")
parser.add_argument("-gdf", help="consider or not consider goodness of Gaussian fitting. True = Consider, False = Don't consider", dest="red_chisq", default = True)
...
input_np = int(args.np)
overwrite = args.overwrite
fit_size = int(args.fit_size)
gdf = args.red_chisq

# input_file = Sp_data[0]
# input_flat = 'Nor_Flat_bin1.fits'
# input_np = 300
# overwrite = 'False'
# fit_size = 70
# gdf = 'False'
...
if os.path.exists(input_file[:-5]+'-1D_extracted_spectrum.csv') == True:
    ...
    if overwrite == 'True':
        ...
        '''extract spectrum started from nth pixel (for LISA only)'''
        x_axis_pixels = np.arange(input_np,len(primary[0]),1)
        ...
        extra_flux_1 = [] 
        extra_flux_1_err = [] # new added on 2021 11 24
...
#             argmax = np.argmax(profile)
            ...
            y_axis_pixels = pd.Series(np.arange(0, len(profile),1))
            ...
            try:
                ...
                '''
                x = y_axis_pixels[argmax-fit_size : argmax+fit_size]
                # y = profile[argmax-50 : argmax+50]
                y = profile_flat_corr[argmax-fit_size : argmax+fit_size]
        ...
        #         upper_background = profile[y_axis_pixels >= upper_background1][y_axis_pixels <= upper_background2].values
                ...
                sigma_background_noise =   np.nanstd(list(lower_background) + list(upper_background))
        ...
        #         signal_in_ap = profile [y_axis_pixels>=lower_aperture][y_axis_pixels<=upper_aperture] - background
                ...
                ccd_gain  = hdus[0].header['EGAIN']
                signal_err_in_ap_Sum = np.sqrt(sigma_background_noise * len(signal_in_ap) + signal_in_ap_Sum/ccd_gain)
                print(v, signal_in_ap_Sum, signal_err_in_ap_Sum)
            ...
            #             extra_flux.append(np.nan)
                        ...
                        extra_flux_1_err.append(np.nan)
                    else:
            #             extra_flux.append(signal_in_ap_Sum)
                        extra_flux_1.append(signal_in_ap_Sum)
                        extra_flux_1_err.append(signal_err_in_ap_Sum)

                if gdf == 'False':

                    if signal_in_ap_Sum <= 0:
            #             extra_flux.append(np.nan)
                        extra_flux_1.append(np.nan)
                        extra_flux_1_err.append(np.nan)
                    else:
        ...
        re1 = pd.DataFrame([], columns=['x-axis-pixel','count/s','count_err/s','lower_Aperture','upper_Aperture'])
        re1['x-axis-pixel'] = pd.Series(x_axis_pixels)
        re1['count/s'] = pd.Series(extra_flux_1)  / hdus[0].header['EXPTIME']
        re1['count_err/s'] = pd.Series(extra_flux_1_err) / hdus[0].header['EXPTIME']
...
#     wavelength = re1['wavelength(\\AA)'].values
    ...
    '''extract spectrum from nth pixel (for LISA only)'''
    x_axis_pixels = np.arange(input_np,len(primary[0]),1)
...
#         argmax = np.argmax(profile)
    ...
    #         signal_in_ap = profile [y_axis_pixels>=lower_aperture][y_axis_pixels<=upper_aperture] - background
        ...
        #             extra_flux.append(signal_in_ap_Sum)
                    extra_flux_1.append(signal_in_ap_Sum)
                    extra_flux_1.append(signal_err_in_ap_Sum)
            if gdf == 'False':
