...
for ind,v in enumerate(flat_only.files):
    ...
    for i in range(len(primary_flat[0])):
        ...
        y = profile_flat[np.abs(argmax_flat-1000) : argmax_flat+1000]  + (-np.nanmin(profile_flat[np.abs(argmax_flat-1000) : argmax_flat+1000]))
        y_min  = np.min(y)
        mean = sum(x*y)/sum(y)
        sigma = np.sqrt(sum(y * (x - mean)**2) / sum(y))

        try:
	        popt,pcov = curve_fit(Gauss, x, y, p0=[max(y), mean, sigma, y_min], maxfev =50000)
